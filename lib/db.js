...
function migrate(db) {
  ...
  try { db.exec("ALTER TABLE wati_sources ADD COLUMN assigned_callers TEXT DEFAULT '[]'"); } catch(e) {}
...
function upsertLead({ phone, name, message, source_number, timestamp }) {
  ...
  if (existing) {
    ...
    // ── Auto-Assignment Logic (Load balancing / Source-based rules) ──
    let assignee = null;
    const assignmentMode = getSetting('assignment_mode', 'global');
    let allowedTelecallers = [];

    if (assignmentMode === 'source' && source_number) {
      const srcRow = db.prepare('SELECT assigned_callers FROM wati_sources WHERE phone = ?').get(srcClean);
      if (srcRow && srcRow.assigned_callers) {
        try {
          allowedTelecallers = JSON.parse(srcRow.assigned_callers);
        } catch(e) {}
      }
    }

    // Default to global active telecallers if global mode OR if no specific callers were assigned to this source
    let sqlCondition = "t.active = 1 AND t.role = 'telecaller'";
    let sqlParams = [];

    if (assignmentMode === 'source' && allowedTelecallers.length > 0) {
      // Create a comma-separated list of params for the IN clause
      const placeholders = allowedTelecallers.map(() => '?').join(',');
      sqlCondition += ` AND t.id IN (${placeholders})`;
      sqlParams = allowedTelecallers;
    }

    // Check if there are ANY telecallers available under the current condition
    const checkCallers = db.prepare(`SELECT id FROM telecallers t WHERE ${sqlCondition}`).all(...sqlParams);

    if (checkCallers.length > 0) {
      // Find the least-loaded telecaller from the allowed pool
    ...
    } else if (assignmentMode === 'source' && allowedTelecallers.length > 0) {
       // If source mode is strict but the allowed callers are all inactive/deleted, fallback to global
       const fallbackLoad = db.prepare(`
        SELECT t.id, COUNT(l.id) as pending_count
        FROM telecallers t
        LEFT JOIN leads l ON l.assigned_to = t.id AND l.status NOT IN ('closed','converted','not_interested')
        ...
        LIMIT 1
      `).get();
      if (fallbackLoad) assignee = fallbackLoad.id;
    }
...
function getAnalytics() {
  ...
  const perSource = db.prepare(
    "SELECT source_number, source_label, COUNT(*) as count FROM leads WHERE source_number != '' GROUP BY source_number ORDER BY count DESC LIMIT 10"
  ).all();
...
function getSourceNumbers() {
  const db = getDb();
  return db.prepare(
    "SELECT DISTINCT source_number, source_label FROM leads WHERE source_number != '' ORDER BY source_label DESC, source_number"
  ).all();
}
...
function updateWatiSourceCallers(id, callersArray) {
  const db = getDb();
  db.prepare('UPDATE wati_sources SET assigned_callers = ? WHERE id = ?').run(JSON.stringify(callersArray), id);
}

// ── Export ──

function getAllLeadsForExport({ startDate, endDate, source, status, assigned_to } = {}) {
  const db = getDb();
  let where = [];
  let params = [];
  ...
  return db.prepare(`
    SELECT l.id, l.phone, l.name, l.status, l.source_number, l.source_label,
...
module.exports = {
  ...
  getWatiSources, addWatiSource, deleteWatiSource, getWatiSourceLabel, updateWatiSourceCallers,
  deleteAllLeads, fullSystemReset, getSetting, setSetting
};
