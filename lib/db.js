...
function upsertLead({ phone, name, message, source_number, timestamp }) {
  ...
  const srcRow = srcClean ? db.prepare('SELECT label, assigned_callers FROM wati_sources WHERE phone = ?').get(srcClean) : null;
  ...
  // ── New lead: determine which telecaller to assign ──
  // If the source number has specific telecallers assigned, route to least-loaded among them.
  // Otherwise fall back to least-loaded across all active telecallers.
  let assignee = null;
  let allowedIds = [];

  if (srcRow && srcRow.assigned_callers) {
    try { allowedIds = JSON.parse(srcRow.assigned_callers); } catch(e) {}
  }

  // Filter allowed list to only active telecallers
  const activeAllowed = allowedIds.length > 0
    ? db.prepare(`SELECT id FROM telecallers WHERE id IN (${allowedIds.map(() => '?').join(',')}) AND active = 1 AND role = 'telecaller'`).all(...allowedIds)
    : [];

  if (activeAllowed.length > 0) {
    const ids = activeAllowed.map(t => t.id);
    ...
    // Global fallback: least-loaded active telecaller
    const load = db.prepare(`
      SELECT t.id, COUNT(l.id) as pending_count
      FROM telecallers t
      LEFT JOIN leads l ON l.assigned_to = t.id AND l.status NOT IN ('closed','converted','not_interested')
...
function getAnalytics() {
  ...
  const perSourceDetailed = db.prepare(`
    SELECT
      source_number, source_label,
      COUNT(*) as total,
      SUM(CASE WHEN status = 'new' THEN 1 ELSE 0 END) as new_leads,
      SUM(CASE WHEN status = 'contacted' THEN 1 ELSE 0 END) as contacted,
      SUM(CASE WHEN status = 'interested' THEN 1 ELSE 0 END) as interested,
      SUM(CASE WHEN status = 'not_interested' THEN 1 ELSE 0 END) as not_interested,
      SUM(CASE WHEN status IN ('follow_up_1','follow_up_2','interested_no_convert') THEN 1 ELSE 0 END) as call_later,
